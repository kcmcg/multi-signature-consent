...
class MultiSignatureConsent extends \ExternalModules\AbstractExternalModule {
    ...
    private static $CURRENT_PDF_KEY = false;
	...
	public function initialize() {
        ...
        foreach ($instances as $pdfKey => $instanceDetails) {
            $this->inputForms[$pdfKey] = $instanceDetails;
        }
	...
	public function redcap_pdf ($project_id, $metadata, $data, $instrument = NULL, $record = NULL, $event_id = NULL, $instance = 1 ) {
        if (self::$MAKING_PDF) {
            ...
            $pdfKey = self::$CURRENT_PDF_KEY;

            // We were called from inside of this EM
            $this->emDebug("In PDF Hook!", func_get_args(), $this->inputForms[$pdfKey], $this->evalLogic[$pdfKey]);
            ...
            foreach ($Proj->metadata as $field_name => $field_meta) {
                if (in_array($field_meta['form_name'], $this->inputForms[$pdfKey])) {
	...
	public function redcap_save_record( $project_id, $record, $instrument, $event_id, $group_id = NULL, $survey_hash = NULL, $response_id = NULL, $repeat_instance = 1) {
        try {
            ...
            foreach($this->evalLogic as $pdfKey => $thisEvalLogic) {
                // Make sure we are in one of the input forms
                if (!in_array($instrument, $this->inputForms[$pdfKey])) {
                    $this->emDebug("$instrument is not in " . implode(",", $this->inputForms[$pdfKey]) . " -- skipping");
                    continue;
                }

                $this->emDebug("Saving $record on $instrument, event $event_id with logic $thisEvalLogic");
                // $event_name = $Proj->longitudinal ? \REDCap::getEventNames(true,true,$event_id) : null;
                $logic = \REDCap::evaluateLogic($thisEvalLogic, $project_id, $record, $event_id);

                if (empty($thisEvalLogic) || $logic == false) {
                ...
                self::$CURRENT_PDF_KEY = $pdfKey;

                // Always start with the 'first form' as the template
                $first_form = $this->inputForms[$pdfKey][0];
                $last_form = $this->inputForms[$pdfKey][count($this->inputForms[$pdfKey]) -1 ];

                $pdf        = \REDCap::getPDF($record, $first_form, $event_id, false, $repeat_instance,
                        true, $this->header[$pdfKey], $this->footer[$pdfKey]);
                ...
                if ($this->saveToExternalStorage[$pdfKey]) {
                    ...
                    \REDCap::logEvent($this->getModuleName(), "A PDF (" .
                            ...
                            implode(",", $this->inputForms[$pdfKey]), "", $record, $event_id);
                ...
                if (!empty($this->destinationFileField[$pdfKey])) {
                    $data = [
                            $record => [
                                    $event_id => [
                                            $this->destinationFileField[$pdfKey] => $edoc_id
                                    ]
                            ]
                    ];
                    ...
                    \REDCap::logEvent($this->getModuleName(), $this->destinationFileField[$pdfKey] .
                ...
                if ($this->saveToFileRepo[$pdfKey]) {
                    $pdf_form = empty($this->saveToAsSurvey[$pdfKey]) ? $last_form : $this->saveToAsSurvey[$pdfKey];
